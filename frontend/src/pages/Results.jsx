...
export default function Results() {
    ...
    return (
      <div className="min-h-[70vh] flex items-center justify-center text-slate-300 font-sans">
        <div className="text-center">
          ...
          <button onClick={() => navigate('/configure')} className="px-5 py-2 rounded bg-white text-slate-950 font-bold cursor-pointer">CREATE SCENARIO</button>
        </div>
      </div>
    );
  }
  ...
  const windDirection = Number(estimate.wind?.direction_from_deg ?? envForecast.wind_direction_deg ?? scenario.environment?.wind_direction_deg ?? 135);
  const windSpeed = Number(estimate.wind?.speed_m_s ?? envForecast.wind_speed_ms ?? scenario.environment?.wind_speed_mps ?? 8);
  ...
  const tankVolume = hazardPrediction?.tank_volume_m3 ?? Math.round(Math.PI * Math.pow(scenario.facility.diameter_m / 2, 2) * scenario.facility.height_m);
  const recommendedApproach = estimate.lower_hazard_approach_direction ?? 'SW';

  // --- 2. Extract Discrete Grid Square GeoJSON ---
  // estimate.geojson contains the square polygon grid calculated by /api/v1/estimate
  const gridGeoJson = estimate.geojson?.type === 'FeatureCollection'
    ? estimate.geojson
  ...
  return (
    <div className="min-h-[calc(100vh-100px)] pb-6 font-sans text-gray-200 max-w-7xl mx-auto px-4">
      ...
      <div className="flex flex-col xl:flex-row xl:items-end xl:justify-between gap-3 border-b border-slate-800 pb-3 mb-4">
        <div>
          <div className="text-[10px] font-mono tracking-[.25em] text-red-400 mb-1">THERMAVECTOR / CELLULAR GRID ESTIMATION</div>
          <h2 className="text-2xl font-black text-white uppercase tracking-tight">Discrete Grid Exposure Matrix</h2>
          <p className="text-xs text-slate-500 mt-1">
            25m cell grid resolution · {cellCount} active exposure cells · {latitude.toFixed(4)}, {longitude.toFixed(4)}
          </p>
        </div>
        <div className="flex flex-wrap gap-2 text-[10px] font-mono">
          <span className="px-3 py-1.5 rounded border border-green-900 bg-green-950/30 text-green-300">● GRID GENERATED</span>
          ...
          {threatLevel && (
            <span className={`px-3 py-1.5 rounded border font-bold ${threatLevel === 'CRITICAL' ? 'border-red-900 bg-red-950/50 text-red-400 animate-pulse' : 'border-yellow-900 bg-yellow-950/40 text-yellow-300'}`}>
              THREAT: {threatLevel}
            </span>
          )}
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,3.3fr)_minmax(300px,1fr)] gap-3">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden relative min-h-[620px] shadow-xl">
          <MapView
            ...
            rescueRoute={hazardPrediction?.rescue_route || []}
          />
        </div>
        ...
        <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2.5">
              ...
              <div className="bg-red-950/30 border border-red-900/60 rounded-lg p-3 flex flex-col justify-between relative overflow-hidden">
                ...
                <div>
                  ...
                  <div className="text-lg font-black text-white font-mono mt-1">
                    {severityZones.find(z => z.severity === 'critical')?.cell_count ?? 0} <span className="text-xs text-red-400 font-normal">cells</span>
                  </div>
                </div>
              ...
              <div className="bg-orange-950/30 border border-orange-900/60 rounded-lg p-3 flex flex-col justify-between relative overflow-hidden">
                ...
                <div>
                  ...
                  <div className="text-lg font-black text-white font-mono mt-1">
                    {severityZones.find(z => z.severity === 'high')?.cell_count ?? 0} <span className="text-xs text-orange-400 font-normal">cells</span>
                  </div>
                </div>
              ...
              <div className="bg-yellow-950/30 border border-yellow-900/60 rounded-lg p-3 flex flex-col justify-between relative overflow-hidden">
                ...
                <div>
                  ...
                  <div className="text-lg font-black text-white font-mono mt-1">
                    {severityZones.find(z => z.severity === 'moderate')?.cell_count ?? 0} <span className="text-xs text-yellow-400 font-normal">cells</span>
                  </div>
                </div>
              ...
              <div className="bg-emerald-950/30 border border-emerald-900/60 rounded-lg p-3 flex flex-col justify-between relative overflow-hidden">
                ...
                <div className="mt-2 text-[9px] text-emerald-300/80 font-mono border-t border-emerald-900/40 pt-1.5 flex justify-between">
                  <span>VECTOR:</span><span className="font-bold text-green-400">LEAST HAZARD</span>
                </div>
              </div>
            </div>

            {/* Emergency Banner */}
            <div className={`mt-2.5 border rounded-lg p-3 flex items-center gap-3 ${threatLevel === 'CRITICAL' ? 'bg-red-950/50 border-red-600/50' : 'bg-orange-950/50 border-orange-600/50'}`}>
              <div className="text-xl animate-bounce">🚨</div>
              <div className="flex-1">
                <div className={`text-[10px] font-black uppercase tracking-wider font-mono ${threatLevel === 'CRITICAL' ? 'text-red-400' : 'text-orange-400'}`}>
          ...
          <section className="bg-slate-900 border border-slate-800 rounded-xl p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-[9px] font-mono text-green-400 border border-green-900 bg-green-950/30 px-2 py-1 rounded">CALCULATED</span>
            </div>
          ...
          <section className="bg-slate-900 border border-slate-800 rounded-xl p-4">
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-[10px] font-mono">
              <div><span className="text-slate-600">RESOLUTION</span><div className="text-slate-300 mt-0.5">25m Cells (500m Box)</div></div>
              <div><span className="text-slate-600">ALGORITHM</span><div className="text-slate-300 mt-0.5">Point Source Flux Matrix</div></div>
              <div><span className="text-slate-600">VECTOR MATH</span><div className="text-slate-300 mt-0.5">Atmospheric Attenuation</div></div>
              <div><span className="text-slate-600">UNIT</span><div className="text-slate-300 mt-0.5">{unit}</div></div>
            </div>
          </section>

          <button onClick={() => navigate('/configure')} className="w-full py-3 rounded-lg bg-white text-slate-950 font-black text-xs tracking-widest hover:bg-slate-200 transition cursor-pointer">
